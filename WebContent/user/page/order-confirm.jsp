...
<div id="container" class="clear">
    ...
    <div class="adress_choice">
        ...
        <c:forEach items="${list}" var="add">
        	<div id="addresId1">
				<i class="user_choice">
					<span hidden="true">${add.rid}</span>
	                <input type="radio" name="address" value="${add.address}-${add.receiver_phone}"/>
	            </i>
	            <i class="address_name">${add.receiver}</i>
	            <i class="user_address">${add.address}-${add.receiver_phone}</i>
        	</div>
        </c:forEach>
        <a href="address-add.jsp">
          添加地址 &gt;&gt;
        </a>
    </div>
    <!-- 商品信息-->
    <form name="" method="post" action="#">
        <div class="product_confirm">
            ...
            <c:forEach items="${omlist}" var="list">
            	<ul class="item_detail">
               <li class="p_info">
                   <b><img src="../img/goods/${list.isbn}/detail1.jpg"/></b>

                   <b class="product_name lf">
                       ${list.title}(第${list.edition}版)
                   </b>
                   <br/>
               <span class="product_color ">
                     出版社：${list.press}
               </span>
               <span  id="order_id">${list.order_id}</span>
               </li>
               <li class="p_price">
                   <i>达内专属价</i>
                   <br/>
                   <span class="pro_price">￥<span class="ppp_price">${list.price}</span></span>
               </li>
               <li class="p_count">X<span>${list.count}</span></li>
               <li class="p_tPrice">￥
               <span id="money">${list.count*list.price}</span>
               </li>
           </ul>
		...
		if(div==null){
			...
			for(var i=0;i<addresses.length;i++){
				if(addresses[i].checked){
					address_id = addresses[i].previousElementSibling.innerHTML;
					b = true;
				}
			}
			if(b){
				...
				var url="address_id="+address_id+"&money="+money+"&order_id="+order_id;
